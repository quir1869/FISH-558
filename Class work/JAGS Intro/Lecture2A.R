# latest JAGS
# https://sourceforge.net/projects/mcmc-jags/files/rjags/4/

library(runjags)
print(findjags())
Sys.setenv(JAGS_HOME="C:/Program Files/JAGS/JAGS-4.3.1")
runjags.options(jagspath = "C:/Program Files/JAGS/JAGS-4.3.1/x64/bin/jags-terminal.exe")
print(findjags())
library(rjags)
...
results <- run.jags(model=model, monitor=c("coef", "int", "precision","sigma"), 
         data=data, n.chains=5, method="rjags", inits=inits,plots=T,silent.jag=T)
         
print(results)
plot(results, layout = runjags.getOption("plot.layout"),
     new.windows = runjags.getOption("new.windows"), file = "E:/testjags.pdf")
